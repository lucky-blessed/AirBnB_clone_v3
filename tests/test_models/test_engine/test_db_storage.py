...
class TestFileStorage(unittest.TestCase):
    ...
    def test_save(self):
        ...
        state_data = {"name": "Califonia"}
        new_state = State(**state_data)
        models.storage.new(new_state)
        models.storage.save()
        session = models.storage._DBStorage__session
        retrieved_state = session.query(State)filter_by(id=new_state).first()
        self.assertEqual(retieved_state.id, new_s    tate.id)
        self.assertEqual(retrieved_state.name, ne    w_state.name)
        self.assertIsNone(retrieved_state)

    @unittest.skipIf(models.storage_t != 'db', "not testing db storage")
    def test_get(self):
        """ Test method to obtain an instance db storage. """
        storage = models.storage
        storage.reload()
        state_data = {"name": "Benin"}
        state_instance = State(**state_data)
        storage.new(state_instance)
        storage.save()
        retieved_state = storage.get(State, state_instance.id)
    ...
    def test_count(self):
        ...
        city_data = {"name": "Light", "state_id": state_instance.id}
